...
public class DungeonGenerator : MonoBehaviour
{
    ...
    void SpawnDoors()
    {
        if (doorPercent > 0)
        {
            //Get an array of all generated connectors
            Connector[] allConnectors = transform.GetComponentsInChildren<Connector>();
            for (int i = 0; i < allConnectors.Length; i++)
            {
                //Get the collected connectors and check if they are connected to another connector
                Connector myConnector = allConnectors[i];
                if (myConnector.isConnected)
                {
                    //Random chance of spawning a door here, spawns only if the percentage odds are within chosen chance
                    ...
                    if (roll <= doorPercent)
                    {
                        ...
                        //Create an overlap box to check collision
                        Collider[] hits = Physics.OverlapBox(pos + offset, halfExtents, Quaternion.identity, LayerMask.GetMask("Door"));
                       
                        //If does not hit anything, place door frame from doorPrefab list
                        if (hits.Length == 0)
                        {
                            //Gets a random door from the door prefab list
                            int doorIndex = Random.Range(0, doorPrefabs.Length);
                            //Instanciates the door on connector location
    ...
    //Create Exit Tile (Point)
    Transform CreateExitTile()
    {
        //Choses one of the Exit Prefabs at Random
